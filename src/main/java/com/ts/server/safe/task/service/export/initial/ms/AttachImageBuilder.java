 ...
 */
 class AttachImageBuilder implements PageBuilder {
    ...
    private List<String> getImages(InitReportContent report){
        ...
        for(InitReportContent.CheckItem items: report.getSafeProduct().getSceneContents()){
            if(Objects.nonNull(items.getImages())){
                images.addAll(Arrays.asList(items.getImages()));
            }
        }
