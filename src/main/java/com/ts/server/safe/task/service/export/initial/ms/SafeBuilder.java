 ...
 */
 ...
 class SafeBuilder implements PageBuilder<InitReportContent> {
    ...
    private void renderProfile(XWPFTable table, InitReportContent report){
        XWPFTableRow row = table.getRow(0);
        XWPFTableCell cell = row.getCell(0);
    ...
    private void renderBaseDetail(XWPFTable table, InitReportContent.SafeProduct product){
        renderDetail(table, 1,"1、基础管理隐患描述及治理措施", product.getBaseContents());
    }

    private void renderDetail(XWPFTable table, int rowIndex, String title, List<InitReportContent.CheckItem> items){
        ...
        MsUtils.renderTable(contentTable, items, Arrays.asList(
    ...
    private void renderSceneDetail(XWPFTable table, InitReportContent.SafeProduct product){
        renderDetail(table, 3, "2、现场管理隐患描述及治理措施", product.getSceneContents());
    }

    private void renderDisease(XWPFTable table, InitReportContent.SafeProduct product){
    ...
    private void renderSpePerson(XWPFTable table, InitReportContent.SafeProduct product){
    ...
    private void renderResult(XWPFTable table, InitReportContent.SafeProduct product){
    ...
    private void renderSafeRisk(XWPFTable table, InitReportContent.SafeProduct product){
    ...
    private void renderRiskType(XWPFTable table, InitReportContent.SafeProduct product){
