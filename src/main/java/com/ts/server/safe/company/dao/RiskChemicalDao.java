...
public class RiskChemicalDao {
    ...
    public RiskChemicalDao(DataSource dataSource){
        ...
        this.idGenerator = IdGenerators.seqId(
                dataSource, "seq_risk_chemical", e -> String.format("%07d", e));
    }

    public String insert(RiskChemical t){
        String id = idGenerator.generate();
        final String sql = "INSERT INTO c_risk_chemical (id, comp_id, name, cas, alias, max_store, remark, create_time) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, now())";
    ...
    public boolean update(RiskChemical t){
        final String sql = "UPDATE c_risk_chemical SET name = ?, cas = ?, alias = ?, max_store = ?, remark = ? WHERE id = ?";
        return jdbcTemplate.update(sql, t.getName(), t.getCas(), t.getAlias(), t.getMaxStore(), t.getRemark(), t.getId()) > 0;
    }

    public boolean delete(String id){
        final String sql = "DELETE FROM c_risk_chemical WHERE id = ?";
        return jdbcTemplate.update(sql, id) > 0;
    }

    public RiskChemical findOne(String id){
        final String sql = "SELECT * FROM c_risk_chemical WHERE id = ?";
    ...
    public List<RiskChemical> find(String compId, String name, String cas, int offset, int limit){
        final String sql = "SELECT * FROM c_risk_chemical WHERE comp_id LIKE ? AND name LIKE ? AND cas LIKE ? " +
